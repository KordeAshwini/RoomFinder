...
const createBooking = async (req, res) => {
  try {
    ...
    // 🔹 Prevent more than 1 booking for the same property by the same user
    const existingBookingsCount = await Booking.countDocuments({ user: userId, property: propertyId });
    if (existingBookingsCount >= 1) {
      return res.status(400).json({ success: false, error: "You can only book this property up to 1 time." });
    }

    // 🔹 Prevent duplicate active booking for same property by the same user
    const activeBooking = await Booking.findOne({
      user: userId,
      property: propertyId,
      status: { $in: ["Pending", "Accepted"] },
    ...
    // 🔹 Check Availability (Flat vs PG)
    // We count both "Accepted" and "Confirmed" to prevent double-booking while payment is pending.
    const filledSlotsCount = await Booking.countDocuments({
      property: propertyId,
      status: { $in: ["Accepted", "Confirmed"] },
    });

    if (property.propertyType === 'Flat' && (property.typeOfTenant === 'Family' || property.typeOfTenant === 'Single')) {
      // For Flats (Family or Single), strictly 1 booking allowed.
      // If any slot is filled (Accepted or Confirmed), restrict others.
      if (filledSlotsCount >= 1) {
        return res.status(400).json({
          success: false,
          error: "This Flat is already booked/confirmed for a tenant.",
        });
      }
    } else {
      // For PGs, check against the sharing limit
      const maxSharing = property.sharing || 4;
      if (filledSlotsCount >= maxSharing) {
        return res.status(400).json({
          success: false,
          error: `This PG is fully booked (${maxSharing}-sharing limit reached).`,
        });
      }
    }
    ...
    const booking = new Booking({
      ...
      message,
      gender,
      typeOfTenant, // Stores 'Family', 'Single', etc.
    });
    await booking.save();
