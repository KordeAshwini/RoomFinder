...
const BookingFormModal = ({ onClose, pgId, pgName, gender, propertyType, sharing, moveInDate, typeOfTenant }) => {
        ...
        {propertyType==="Flat" && (
          ...
          {/* <div className="relative">
            ...
            <input
              ...
              onChange={handleChange}
              required
              max={moveInDate}   // ✅ Owner's stored date is the minimum
              className="w-full bg-gray-100 border p-2 rounded-md text-sm text-gray-700"
            />
  ...
  <input
    ...
    onChange={handleChange}
    required
    min={moveInDate.split("T")[0]} // ✅ sets calendar to start from stored date
    className="w-full bg-gray-100 border p-2 rounded-md text-sm text-gray-700"
  />
  <div className="absolute right-3 bottom-2.5 text-black pointer-events-none">
    <svg
    ...
    >
      ...
      />
          ...
          {typeOfTenant!=="Family" && typeOfTenant!=="Single"&& (
          <div>
            <label className="text-sm text-gray-700">Sharing Preference : </label>
            <span className="text-orange-600">Available for: {sharing}</span>
            <select
